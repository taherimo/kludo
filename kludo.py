...
def make_graph(aminoacids,aminoacid_ca_coords, co_alpha_helix_matrix, co_beta_strand_matrix,betasheet_labels, acc, hydphob,hydrogen_bonds,beta_bridges):
    ...
    loaded_model = pickle.load(open('edge_weight_predictor/edge-weight-predictor-SMOTEENN.sav', 'rb'))
...
def run(argv):
    ...
    # score_model = pickle.load(open('single-domain-multi-domain-classifier-astral95.sav', 'rb'))
    f = open('single_multi_classifier/SM-Classifier-SMOTEENN-Bagging.sav', 'rb')
    weak_classifiers = []
    for i in range(100):
